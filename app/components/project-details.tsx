      ...
      <article className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        >
          <div className="relative rounded-2xl border border-border overflow-hidden group">
            <div className="relative w-full h-[300px] sm:h-[400px] lg:h-[500px] bg-card">
              {project.images.map((image, index) => (
                <Image
                  key={index}
                  src={image}
                  alt={`${project.title} - Screenshot ${index + 1} showing ${
                    index === 0
                      ? "main interface"
                      : index === 1
                        ? "dashboard view"
                  ...
                  className={`absolute inset-0 w-full h-full object-center object-contain p-2 transition-all duration-700 ${
            ...
            <nav className="space-x-4 p-2" aria-label="Screenshot thumbnails">
              {project.images.map((image, index) => (
                <button
                  key={index}
                  onClick={() => goToImage(index)}
                  className={`flex-shrink-0 cursor-pointer w-16 sm:w-20 h-12 sm:h-16 rounded-lg overflow-hidden transition-all ${
                    index === currentImageIndex
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Key Features */}
            <section
              className="bg-card border border-border rounded-xl shadow-lg p-6 sm:p-8 animate-slide-up"
              ...
              <ul className="space-y-3" itemProp="featureList">
                {project.features.map((feature, index) => (
                  <li key={index} className="flex items-start gap-3">
                    <span
                      className="flex-shrink-0 w-6 h-6 bg-accent text-foreground rounded-full flex items-center justify-center text-sm font-semibold mt-0.5"
                      aria-hidden="true"
                    >
                      {index + 1}
                    </span>
            ...
            {(project.challenges || project.learnings) && (
              <div
                className="grid sm:grid-cols-2 gap-6 animate-slide-up"
                  ...
                  <section className="bg-card border border-border rounded-xl shadow-lg p-6">
                    ...
                    <ul className="space-y-2">
                      {project.challenges.map((challenge, index) => (
                        <li
                          key={index}
                          className="text-foreground text-sm pl-4 border-l-2 border-orange-500"
                        >
                          {challenge}
                        </li>
                      ))}
                  ...
                  <section className="bg-card border border-border rounded-xl shadow-lg p-6">
                    ...
                    <ul className="space-y-2">
                      {project.learnings.map((learning, index) => (
                        <li
                          key={index}
                          className="text-foreground text-sm pl-4 border-l-2 border-green-500"
                        >
                          {learning}
                        </li>
                      ))}
          ...
          <aside className="space-y-6">
            {/* Links */}
            <section
              className="bg-card border border-border rounded-xl shadow-lg p-6 animate-slide-up"
              ...
              <dl className="space-y-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <dd>
                      <span
                        className={`inline-block px-2 py-1 rounded text-xs font-medium ${statusColors[project.deployment.status as keyof typeof statusColors]}`}
                      >
            ...
            >
              ...
              <div className="space-y-4">
                <meta
                  ...
                  content={project.techStack.map((t) => t.name).join(", ")}
                />
              ...
              <dl className="space-y-2">
                <div>
                  ...
                  <dd className="font-medium text-accent">
                    <time dateTime={project.startDate} itemProp="dateCreated">
                ...
                {project.endDate && (
                  <div>
                    ...
                    <dd className="font-medium text-accent">
                      <time dateTime={project.endDate} itemProp="dateModified">
